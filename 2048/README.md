...
`var game = new Game() // generates a random starting board`

`var game = new Game('0000202000000000') // generates a board with the given start`
...
You might want to setup Jasmine and write specs for model. Or you might want to make a simple `game_test.js` file that includes simple assertions (check out `console.assert`). This game has a variety of interesting edge cases to consider, so make sure you have the logic working correctly.

### Release 2: Integrate the model with the interface via a controller

In your application.js file code, instantiate a new instance of the Game model. Write code to allow key presses to send `move` commands to the Game.
